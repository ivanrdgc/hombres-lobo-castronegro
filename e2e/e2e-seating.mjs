...
// arrastrar el asa ⠿ desde el lobby: Dani (asiento 2) baja al final de la lista
await ana.locator('.draghandle[data-drag=p-dani]').hover();
...
const s3 = await seating(ana);
if (s3 && s3[s3.length - 1] === dani) ok('arrastrar el asa reordena y guarda el orden: ' + s3.join(','));
else bad('el arrastre no se guardó: ' + JSON.stringify(s3));
