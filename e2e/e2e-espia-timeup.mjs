// E2E El Espía · el reloj llega a CERO (4 jugadores, 2 rondas; con la semilla
...
//  R2: todos pasan → el espía se va de rositas (+2), regla oficial.
...
try {
  ...
  // ——— RONDA 2: todos pasan → el espía sobrevive (+2) ———
  ...
  for (let turn = 0; turn < order2.length; turn++) {
    ...
    await hp.click('[data-a=espia-tu-pass]');
    await waitState(ana, (s) => s.phase === 'end' || s.timeupTurn === turn + 1, 'pase registrado');
  }
