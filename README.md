...
- **🤖 Automático**: la app dirige la partida sin narrador humano. El dispositivo del máster narra con voz sintetizada (mantenlo encendido y con volumen) y muestra un panel de narrador (paso actual, a quién se espera). Desde 🗣️ Voz el máster puede elegir la voz (las «Natural»/«Online»/Google suenan mucho mejor), la velocidad y el tono, y probarla. Cada rol actúa desde su móvil por turnos; de día cualquiera registra la decisión del pueblo (primera elección gana, se asume buena fe). Los lobos igual: el primero que elige decide por la manada.
- **🎩 Manual**: el máster narra al estilo clásico. La app reparte los roles en secreto, se los muestra todos al máster y lleva la crónica de muertes (devorado/linchado/otros, con opción de corrección).
