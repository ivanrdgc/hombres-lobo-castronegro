<script lang="ts">
  // Cómo se juega Coup: guía por apartados, con un ▶️ de lectura en cada uno
  // (solo en el lobby).
  ...
  const keyOf = (i: number) => 'coup-howto:' + i;
</script>

<h3 style="margin:0 0 4px">🃏 Cómo se juega Coup</h3>
...
{#each HOW_TO as sec, i (i)}
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-top:14px">
    ...
    {#if canPlay}
      {#if st === 'playing'}
        <button class="small ghost" data-a="coup-play-howto" aria-label="Detener" title="Detener" onclick={() => toggleLocalSpeech(keyOf(i), [])}>⏹️</button>
      {:else if st === 'loading'}
        <button class="small ghost" aria-label="Preparando la voz" disabled><span class="spinner"></span></button>
      {:else}
        <button class="small ghost" data-a="coup-play-howto" data-p={String(i)} aria-label="Escuchar este apartado" title="Escuchar" style="font-size:1.05rem;line-height:1" onclick={() => toggleLocalSpeech(keyOf(i), [sec.heading, ...sec.items])}>▶️</button>
      {/if}
    {/if}
  </div>
