<script lang="ts">
  // Lobby de Coup: intro con lectura (▶️) + «cómo se juega». Sin ajustes de
  // roles (la app baraja y reparte). Quién juega se elige en «Empezar».
...
<div class="card">
  <div style="display:flex;align-items:center;gap:8px">
    <h3 style="flex:1;margin:0">🃏 Coup</h3>
    ...
    {:else}
      <button class="small ghost" data-a="coup-play-intro" title="Escuchar" style="font-size:1.1rem;line-height:1" onclick={() => toggleLocalSpeech('coup-intro', INTRO_LOBBY)}>▶️</button>
    {/if}
  </div>
  {#each INTRO_LOBBY as p, i (i)}<p style="margin:9px 0">{p}</p>{/each}
  <button class="block" data-a="coup-open-help" onclick={() => (app.ui.modal = { type: 'coup-help' })}>🎲 Cómo se juega</button>
  <button class="block" data-a="open-demo" onclick={() => (app.ui.modal = { type: 'coup-demo' })}>🎓 Tutorial interactivo (2 min)</button>
</div>

<div class="card">
  <p class="small-note" style="margin-top:0">De {MIN_PLAYERS} a {MAX_PLAYERS} jugadores. La app baraja la corte, custodia las influencias, resuelve los desafíos y bloqueos y lleva las monedas; la voz (opcional) relata cada jugada en alto (nunca tus cartas ocultas).</p>
