<script lang="ts">
  ...
  function startNow() {
    ...
    if (narrator === meId) {
      unlockAudio();
      play({ id: 'unlock', segments: [{ kind: 'clip', text: 'Que empiece Codenames.' }] }).catch(() => {});
      app.ui.voiceUnlocked = true;
    }
    guard(() => A.startCodenames(pids, narrator));
  }
</script>

<div class="topbar">
  <button class="small ghost" data-a="back-lobby-game" aria-label="Volver" title="Volver" style="font-size:1.25rem;line-height:1;padding:6px 12px" onclick={() => navigate(`/g/${group.id}/codenames`)}>←</button>
  <h2>🕵️ Codenames: empezar</h2>
</div>
<Flash />
