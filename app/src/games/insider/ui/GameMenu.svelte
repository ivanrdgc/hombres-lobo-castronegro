  ...
  {#if open}
    ...
    <div class="menu-pop" role="menu">
      ...
      <!-- Dentro de una partida la URL se recoloca sola: sin esta entrada no hay
           ...
           fase espera POR ÉL: sin su carta vista o su voto no se avanza). -->
      <button role="menuitem" data-a="table-open" onclick={() => { app.ui.modal = { type: 'table' }; close(); }}>🪑 La mesa</button>
      {#if playing}
        <button role="menuitem" data-a="ins-repeat" onclick={() => { guard(A.requestRepeat); close(); }}>🔁 Repetir</button>
        {#if game.paused}
          <button role="menuitem" data-a="ins-resume" onclick={() => { guard(A.resumeGame); close(); }}>▶️ Reanudar</button>
        {:else}
          <button role="menuitem" data-a="ins-pause" onclick={() => { guard(A.pauseGame); close(); }}>⏸️ Pausar</button>
        {/if}
      {/if}
      {#if spectator}
      ...
      {/if}
      {#if !spectator}
        <button role="menuitem" class="danger-text" data-a="ins-leave-open" onclick={() => { app.ui.modal = { type: 'ins-leave' }; close(); }}>🚪 Dejar la partida</button>
      {/if}
      <button role="menuitem" class="danger-text" data-a="ins-end-open" onclick={() => { app.ui.modal = { type: 'ins-end' }; close(); }}>🏳️ Terminar</button>
    </div>
  {/if}
</div>
