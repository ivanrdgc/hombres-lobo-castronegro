...
import { WL_INTRO, GUESS_LINE, roundLine, resultLine } from '../texts';
...
function sceneOf(s: Snap): SceneDef<Snap> | null {
  ...
  switch (g.phase) {
    ...
    case 'result': return { key: `${C}:result`, run: resultScene };
    default: return null;
  }
}
...
async function clueScene(ctx: Ctx): Promise<void> {
  const g = gm(ctx);
  if (g.round === 1 && !g.clue) await ctx.sayOnce(`W${g.startedAt}:intro`, () => utt('wl-intro', WL_INTRO));
  await ctx.sayOnce(`W${g.startedAt}:r${g.round}:clue`, () =>
    utt('wl-clue', roundLine(spectrumLabel(g.spectrumId), nm(g, psychicId(g)))));
}

async function guessScene(ctx: Ctx): Promise<void> {
  const g = gm(ctx);
  await ctx.sayOnce(`W${g.startedAt}:r${g.round}:guess`, () => utt('wl-guess', GUESS_LINE));
}
...
async function resultScene(ctx: Ctx): Promise<void> {
  ...
  await ctx.sleep(400);
  await ctx.sayOnce(`W${g.startedAt}:r${g.round}:result`, () =>
