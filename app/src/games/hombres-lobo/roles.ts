...
// Port literal de public/js/roles.js (v1); solo se añaden tipos.

export interface RoleDef {
  id: string;
  name: string;
  emoji: string;
  team: 'pueblo' | 'lobos' | 'solitario';
...
const WOLF_EXTRAS: RoleId[] = ['lobo_feroz', 'infecto', 'lobo_albino'];
...
export function wolfCountFor(n: number): number {
  if (n <= 6) return 1;
  if (n <= 11) return 2;
  if (n <= 17) return 3;
  return 4;
...
export const NEIGHBOR_ROLES: RoleId[] = ['zorro', 'domador', 'caballero', 'sectario'];

export const OFFICIAL_MIN_PLAYERS = 8; // jugadores repartidos, sin contar al narrador
export const CASUAL_MIN_PLAYERS = 3;

export function isWolfTeamRole(roleId: RoleId | string | null | undefined): boolean {
  return roleId === 'hombre_lobo' || roleId === 'lobo_feroz' || roleId === 'infecto' || roleId === 'lobo_albino';
}

/** Forma mínima de jugador que necesitan las utilidades de bando. */
export interface WolfSideish {
  role?: RoleId | null;
  infected?: boolean;
  transformed?: boolean;
...
export function isWolfSide(p: WolfSideish): boolean {
...
export function effectiveTeam(p: WolfSideish): 'pueblo' | 'lobos' | 'solitario' {
...
/** Las 120 combinaciones posibles ("X de Y"): la F6 las pre-genera como clips. */
export function allKeywordCombos(): string[] {
  const combos: string[] = [];
  for (const a of KW_A) for (const b of KW_B) combos.push(`${a} de ${b}`);
  return combos;
}

export function generateKeywords(n: number, seed: number): string[] {
  const rnd = mulberry32(seed >>> 0);
  return shuffled(allKeywordCombos(), rnd).slice(0, n);
}
...
export function buildDeck(
  ...
  extraRoles: RoleId[],
  seed: number,
  wolvesOverride: number | null = null,
  villagersOverride: number | null = null,
): DeckResult {
  ...
  const enabled = extraRoles.filter((r) => ROLES[r] && !(ROLES[r] as RoleDef).always);
  const dropped: { id: RoleId; reason: string }[] = [];
  const extras: RoleId[] = [];
  for (const r of enabled) {
    const min = (ROLES[r] as RoleDef).minPlayers;
    if (min && n < min) dropped.push({ id: r, reason: 'min' });
    else extras.push(r);
  }

  // Bando lobo: los especiales sustituyen lobos comunes.
  const wolfSpecials = shuffled(extras.filter((r) => WOLF_EXTRAS.includes(r)), rnd);
  const wolfDeck: RoleId[] = wolfSpecials.slice(0, wolves);
  ...
  const villageDeck: RoleId[] = [];
  for (const r of villageSpecials) {
    const copies = (ROLES[r] as RoleDef).multi || 1;
  ...
  let pool: RoleId[] = wolfDeck.concat(villageDeck);
...
export function dealRoles(
  ...
  const composition: Partial<Record<RoleId, number>> = {};
  for (const r of pool) composition[r] = (composition[r] || 0) + 1;
  return { assignments, center, composition, dropped };
}

/** Forma mínima de jugador para el cálculo de vecinos. */
export interface Seatish {
  id: string;
  order?: number;
  alive?: boolean | null;
}
...
export function aliveNeighbors<T extends Seatish>(playersArr: T[], pid: string): T[] {
