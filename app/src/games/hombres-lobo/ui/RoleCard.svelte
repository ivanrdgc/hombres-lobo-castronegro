<script lang="ts">
  ...
  // de la v1).
  //
  // UNA SOLA PUERTA (B34): esta tarjeta NUNCA se dibuja en el cuerpo de una
  // pantalla de partida. Vive detrás de la pastilla flotante «🎴 Mi carta»
  // (shell/CardFab → MyInfoModal), que es idéntica en los 12 móviles de la
  // mesa, y detrás de la puerta del reparto (RevealGate). Antes cada pantalla
  // añadía además su propio «👁 Ver mi carta» plegable: dos botones para lo
  // mismo, con dos nombres distintos. Quien la muestra es quien se encarga de
  // ocultarla (ambos contenedores tienen su auto-ocultado de 12 s).
  ...
  const { player, group }: { player: PlayerDoc; group: GroupDoc } = $props();
  ...
  // poder. El dueño abre «🎴 Mi carta» con disimulo y actúa desde aquí.
  ...
  <div class="rolecard">
    ...
    {#if game.phase === 'day' && player.role === 'juez' && player.alive && player.powers?.juez !== false && !game.powersLost}
      <div class="rextra">⚖️ <b>Tu poder secreto</b>: puedes exigir una segunda votación hoy (una vez por partida). Nadie sabrá que fuiste tú.
        <button class="violet block" data-a="juez-arm" style="margin-top:6px"
          onclick={() => guard(A.armJuez)}>⚖️ Exigir segunda votación tras el juicio</button></div>
    ...
    {#if pendHead?.type === 'sirvienta' && player.role === 'sirvienta' && player.alive && !player.lover}
      ...
      <div class="rextra">🧹 <b>Decide en secreto</b>: el pueblo ha condenado a <b>{condemned?.name || ''}</b>. ¿Sacrificas tu carta para asumir su rol? <Countdown deadline={pendHead.deadline ?? 0} />
        <span class="small-note">Si la carta se cierra sola, vuelve a abrirla en 🎴 Mi carta: sigues a tiempo mientras corra la cuenta.</span>
        <div class="btnrow" style="margin-top:6px">
          <button class="violet" data-a="sirvienta-yes" onclick={() => guard(() => A.resolveSirvienta(true))}>🧹 Tomar su rol</button>
          <button class="ghost" data-a="sirvienta-no" onclick={() => guard(() => A.resolveSirvienta(false))}>No intervenir</button></div></div>
    {/if}
  </div>
{/if}
