...
<div class="card">
  ...
  <div class="chips">
    {#each list as [r, n] (r.id)}
      <button class="chip rolechip" data-a="role-detail" data-p={r.id} onclick={() => open(r.id)}>{r.emoji} {r.name}{n > 1 ? ` ×${n}` : ''}</button>
    {/each}
    {#if alguacilOn}<button class="chip rolechip" data-a="role-detail" data-p="alguacil" onclick={() => open('alguacil')}>⭐ Alguacil</button>{/if}
  </div>
  <p class="small-note">{pub ? 'Composición pública: estas son las cartas en juego.' : 'Composición secreta: roles activados al empezar; nadie sabe cuáles juegan de verdad… ni cuántos.'} Toca una carta para ver cómo funciona.</p>
</div>
