...
{:else if stepId === 'enamorados'}
  ...
  <div class="actionpanel"><h3>💘 Estás enamorado/a</h3>
    ...
    {:else}
      <p class="hint">✅ Confirmado.{#if my.keyword} Tu palabra clave{my.kwRenewedNight === game.night ? ' (renovada)' : ''}: <b>«{my.keyword}»</b>. La tienes siempre en tu carta (🎴 Mi carta, abajo).{/if}</p>
    {/if}
  </div>
...
{:else if stepId === 'vidente'}
  {#if game.acts.videnteTarget !== undefined && game.acts.videnteTarget !== null && !game.acts.videnteSeen}
    ...
    <div class="actionpanel"><h3>🔮 La Vidente</h3>
      ...
      <p class="small-note">Queda anotado en tu carta (🎴 Mi carta, abajo): no hace falta que lo memorices.</p>
    </div>
...
{:else if stepId === 'infectado'}
  ...
  <div class="actionpanel"><h3>🧛 El Infecto Padre de los Lobos te ha mordido</h3>
    ...
    {:else}
      <p class="hint">✅ Confirmado. Desde la próxima noche despiertas y cazas con la manada.{#if my.keyword} Tu palabra clave{my.kwRenewedNight === game.night ? ' (renovada)' : ''}: <b>«{my.keyword}»</b>. La tienes siempre en tu carta (🎴 Mi carta, abajo).{/if}</p>
    {/if}
  </div>
