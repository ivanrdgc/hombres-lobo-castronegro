...
<div class="card">
  <div style="display:flex;align-items:center;gap:8px">
    ...
    {#if introAudio === 'playing'}
      <button class="small ghost" data-a="ll-play-intro" title="Detener" onclick={() => toggleLocalSpeech('ll-intro', [])}>⏹️</button>
    {:else if introAudio === 'loading'}
      <button class="small ghost" aria-label="Preparando la voz" disabled><span class="spinner"></span></button>
    {:else}
      <button class="small ghost" data-a="ll-play-intro" title="Escuchar" style="font-size:1.1rem;line-height:1" onclick={() => toggleLocalSpeech('ll-intro', INTRO_LOBBY)}>▶️</button>
    {/if}
  </div>
  {#each INTRO_LOBBY as p, i (i)}<p style="margin:9px 0">{p}</p>{/each}
  <button class="block" data-a="ll-open-help" onclick={() => (app.ui.modal = { type: 'll-help' })}>🎲 Cómo se juega</button>
  <button class="block" data-a="open-demo" onclick={() => (app.ui.modal = { type: 'll-demo' })}>🎓 Tutorial interactivo (2 min)</button>
</div>
