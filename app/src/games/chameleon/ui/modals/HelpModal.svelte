<script lang="ts">
  // Cómo se juega El Camaleón: guía por apartados, con un ▶️ de lectura en cada
  ...
  const keyOf = (i: number) => 'ch-howto:' + i;
</script>

<h3 style="margin:0 0 4px">🦎 Cómo se juega El Camaleón</h3>
...
{#each HOW_TO as sec, i (i)}
  ...
  <div style="display:flex;align-items:center;gap:8px;margin-top:14px">
    ...
    {#if canPlay}
      {#if st === 'playing'}
        <button class="small ghost" data-a="ch-play-howto" aria-label="Detener" title="Detener" onclick={() => toggleLocalSpeech(keyOf(i), [])}>⏹️</button>
      {:else if st === 'loading'}
