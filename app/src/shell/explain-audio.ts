...
import { explainSpeechPieces } from '../games/hombres-lobo/texts/explain';
...
export function toggleExplainAudio(group: ExplainGroup, part: LocalPart): void {
  ...
  // Pieza a pieza, con su pausa REAL delante (gap): así el título respira antes
  // del primer párrafo aunque no lleve punto (la síntesis sola no pausaría).
  const pieces = explainSpeechPieces(group, part);
  play({
    id: 'explain:' + part,
    priority: 'normal',
    segments: pieces.flatMap<Segment>((p, i) => (i
      ? [{ kind: 'gap', ms: p.pause }, { kind: 'clip', text: p.t }]
      : [{ kind: 'clip', text: p.t }])),
  }, {
    onSegment: (i) => {
