// Tests del motor puro (roles + engine) — se ejecutan con `node --test tests/`
...
test('buildDeck: si no caben, se descartan (dropped)', () => {
  const extras = ['vidente', 'bruja', 'cazador', 'cupido', 'defensor', 'anciano', 'zorro', 'cuervo', 'gitana', 'tonto'];
...
test('turno fantasma: disimula roles muertos (ocultos) y roles vivos sin poder', async () => {
  const { stepNeedsGhostAnnounce } = await import('../public/js/engine.js');
...
test('annotateDeaths: ocultar causas nocturnas', async () => {
  const { annotateDeaths } = await import('../public/js/engine.js');
